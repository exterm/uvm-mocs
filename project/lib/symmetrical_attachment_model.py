...
def generate_network(N: int, C: int, m: int, alpha: float, dump_graphs: bool = False) -> nx.DiGraph:
    ...
    for step in range(len(G.nodes()), N): ## one step for each node to add, we already have 2 nodes (id 0 and 1)
        for _ in range(m): ## one step for each edge to add
            ...
            if rand < alpha: ## do we add an edge from new to old?
                ...
                in_degrees = [G.in_degree(node) + C for node in eligible_nodes]
                ...
                out_degrees = [G.out_degree(node) + C for node in eligible_nodes]
                if len(eligible_nodes) > 0:
